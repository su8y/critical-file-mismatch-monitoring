...
void flatten_json(const json &j, const std::string &prefix,
	...
	try {
		...
		for (const auto &pair1 : flattened_map1) {
			...
			if (it2 != flattened_map2.end()) {
				...
				if (value1 != value2) {
					output << "[CHANGED]_" << key
						   << ":_PREV_VALUE=" << to_string(value1)
						   << "_&_CURRENT_VALUE=" << to_string(value2)
						   << std::endl;
				}
			} else {
				// 키가 map2에 없는 경우: 삭제됨
				output << "[REMOVED]_" << key << ":_" << to_string(value1)
					   << std::endl;
			}
		}
		...
		for (const auto &pair2 : flattened_map2) {
			...
			if (it1 == flattened_map1.end()) {
				// 키가 map1에 없는 경우: 추가됨
				output << "[ADDED]___" << key << ":_" << to_string(pair2.second)
					   << std::endl;
			}
		}
