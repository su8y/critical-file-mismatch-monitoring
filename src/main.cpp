...
{
	spdlog::debug("Sentinel Process: {} {} {} {} {} {}", s.checkfile, s.reffile, s.file_type, s.compare_type,
				  s.compare_trigger_msg, s.compare_time);
	...
	if (s.file_type == "listfile")
	{
		...
		if (checksum_status.contains(s.checkfile) == false)
		{
...
void load_alarm_status_map()
{
	...
	for (auto& [key, value] : j.items())
	{
		try
		{
			...
			spdlog::debug("Load alarm[{}] -> severity: {}, alarm_id: {}", key, status.severity, status.alarm_id);
		}
